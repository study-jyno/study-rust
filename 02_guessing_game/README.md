...
접근하기 위한 방법을 제공하는 _참조자_ 임을 나타냅니다.
...
`Result` 타입은 [_열거형 (enumerations)_][enums]<!-- ignore --> 으로써
줄여서 _enums_ 라고 부르기도 합니다. 열거형은 정해진 값들만을 가질 수 있으며
...
`rand`를 사용하는 코드를 작성하기 전에 _Cargo.toml_ 을
...
_Cargo.toml_ 파일에서 하나의 절의 시작 이후의 모든 내용은
...
(때때로 _SemVer_ 라고 불리는) 을 이용합니다. `0.5.5`는 실제로는 `^0.5.5`의
...
데이터의 복사본인 _레지스트리(registry)_ 에서 모든 것들을 가져옵니다.
...
_Cargo.toml_ 을 다음과 같이 업데이트해야 합니다.
...
다음 arm의 패턴인 `Ordering::Greater`는 _확실히_
...
에러의 핵심은 _일치하지 않는 타입_ 이 있다고 알려 주는 것입니다.
...
만약 `parse`가 문자열을 정수로 바꾸지 _못했다면_
